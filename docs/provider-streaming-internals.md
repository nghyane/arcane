...
This document explains how token/tool streaming is normalized in `@nghyane/pi-ai`, then propagated through `@nghyane/pi-agent-core` and `coding-agent` session events.
...
## Unified stream contract in `@nghyane/pi-ai`
