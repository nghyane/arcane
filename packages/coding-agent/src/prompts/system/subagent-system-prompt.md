...
- Do NOT abort due to uncertainty or missing info that can be obtained via tools or repo context. Use `codemode.find()`/`codemode.grep()`/`codemode.read()` first, then proceed with reasonable defaults if multiple options are acceptable.
