...
RUN mkdir -p /root/.local/bin && \
    cp packages/coding-agent/dist/arc /root/.local/bin/ && \
    cp packages/natives/native/arcane_natives.linux-x64-modern.node /root/.local/bin/ && \
    cp packages/natives/native/arcane_natives.linux-x64-baseline.node /root/.local/bin/
