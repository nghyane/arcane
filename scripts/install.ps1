...
# Usage: irm https://raw.githubusercontent.com/nghyane/arcane/main/scripts/install.ps1 | iex
#
# Or with options:
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/nghyane/arcane/main/scripts/install.ps1))) -Source
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/nghyane/arcane/main/scripts/install.ps1))) -Binary
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/nghyane/arcane/main/scripts/install.ps1))) -Source -Ref v3.20.1
...
$InstallDir = if ($env:ARCANE_INSTALL_DIR) { $env:ARCANE_INSTALL_DIR } else { "$env:LOCALAPPDATA\arcane" }
$BinaryName = "arcane-windows-x64.exe"
$NativeAddonNames = @("arcane_natives.win32-x64-modern.node", "arcane_natives.win32-x64-baseline.node")
...
function Configure-BashShell {
    try {
        $settingsDir = Join-Path $env:USERPROFILE ".arcane\agent"
...
function Install-ViaBun {
    ...
    if ($Ref) {
        ...
        $tmpRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("arcane-install-" + [System.Guid]::NewGuid().ToString("N"))
    ...
    Write-Host ""
    Write-Host "✓ Installed arcane via bun" -ForegroundColor Green

    Configure-BashShell

    Write-Host "Run 'arc' to get started!"
}
...
function Install-Binary {
    ...
    $OutPath = Join-Path $InstallDir "arcane.exe"
    ...
    Write-Host ""
    Write-Host "✓ Installed arcane to $OutPath" -ForegroundColor Green
    ...
    if ($needsRestart) {
        Write-Host "Restart your terminal, then run 'arc' to get started!"
    } else {
        Write-Host "Run 'arc' to get started!"
    }
}
